...
class clientList:
    def __init__(self, path="ClientContact.csv"):
        accountlist=pd.read_csv(path, encoding='cp1252')
        self.accounts=list(set(accountlist['CUSTOMER']))
...
class CreateDoc:
    ...
    #slaByIssue=pd.DataFrame(columns=col)
        ...
        return slaByIssue, slaByPriority           
    ...
    def writeDoc(self,Client, table_issuesla, table_plevelsla,tklist):
...
#        title=createDocTilte()
    ...
    def GenerateDoc(self,client, tklist):
        ...
        self.writeDoc(client,table_issuesla,table_plevelsla,tklist)            
...
#for Janurary report, CHECK REPORTYEAR NUMBER IN _INIT__():
    ...
    for c in clients.accounts:
       if len(tk[(tk['AccountName']==c.strip())])>0:
            client=c.strip()
            try:
                docs.GenerateDoc(client,tklist_df)
                print(c,"report made",)
            except:
